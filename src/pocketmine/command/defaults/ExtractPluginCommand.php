...
class ExtractPluginCommand extends VanillaCommand{
	public function __construct(string $name){
		parent::__construct($name, "Extracts the source code from a Phar plugin", '/extractplugin <pluginName>', ["ep"], [
				[
					new CommandParameter("plugin", AvailableCommandsPacket::ARG_TYPE_RAWTEXT, false)
				]
			]);
		$this->setPermission("pocketbetter.command.extractphar");
	}
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		$folderPath = Server::getInstance()->getPluginPath() . "PocketBetter" . DIRECTORY_SEPARATOR . $description->getFullName() . DIRECTORY_SEPARATOR;
